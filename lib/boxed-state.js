...
function isValidBox(box) {
    return util.isFunction(box) && box.hasOwnProperty('boxedContext') && util.isObjectLike(box.boxedContext) && box.boxedContext.constructor === boxedImmutable.BoxedContext;
}
...
function boxOptions(options) {
    this.boxed = UNDEFINED;
    this.boxOnlyOptions = boxedImmutable.getBoxOnlyOptions(options);
...
function BoxedState(getState, saveState, options) {
    ...
    if (isValidBox(options)) {
        ...
        boxOnlyOptions = util.copyFiltered.call(undefined,options, boxedImmutable.BOX_ONLY_PROPERTIES);
    }
    ...
    this.boxOnlyOptions = util.hasOwnProperties.call(boxOnlyOptions) && boxOnlyOptions;
...
function isBoxedState(arg) {
    return util.isObjectLike(arg) && arg.constructor === BoxedState;
}
...
const BoxedStateHandler = {
    ...
    ownKeys: function (target) {
        if (isBoxedState(target)) {
            ...
            return util.deleteItems.call(keys, target.saveBoxedProp, target.cancelBoxedProp, target.boxOptionsProp);
        }
        throw new TypeError("BoxedStateHandler: IllegalArgument expected BoxedState target");
    },

...
const toIntegerOrDefault = util.toIntegerOrDefault;
const toArrayIndexOrDefault = util.toArrayIndexOrDefault;
...
function boxedContext(options) {
    if (util.isObjectLike(options) && options.constructor === BoxContext) return options;
...
function getWithBoxedOut(value) {
    const newInstance = isObjectLike(value)  ? isArray(value) ? boxOut([]) : boxOut({}) : UNDEFINED;  // pure objects for proxy use
    return newInstance;
}
...
Box.prototype.copyOfValue = function copyOfValue() {
    return util.cloneArrayObject.call(this.value);
};
...
function applyTransforms(value, transforms, boxedArrayEnd, applyFunctions, applyObjects, markModified) {
    ...
    function setValue(prop, propValue) {
        if (value[prop] !== propValue) {
            if (!isCopy) {
                value = util.cloneArrayObject.call(value);
                isCopy = true;
            }
...
Box.prototype.set = function (prop, value, boxed) {
    ...
    if (this.setTransforms) {
        ...
        if (transform || defaultTransform) {
            if (isArrayEnd) {
                prop = util.arrayLength.call(this.value);
            }
    ...
    if (isArrayEnd) {
        if (prop === this.boxedArrayEnd) prop = util.arrayLength.call(this.value);
    ...
    if (this.extraValues) {
        ...
        util.eachProp.call(this.extraValues, (value, key) => {
...
Box.prototype.ifValueObjectBoxedOut = function (callback) {
    return this.ifImpl(false, util.isObjectLike(this.value), arguments);
};

Box.prototype.ifValueObjectBoxedIn = function (callback) {
    return this.ifImpl(true, util.isObjectLike(this.value), arguments);
};
...
Box.prototype.valueObjectBoxedOut = function (callback) {
    if (util.isObjectLike(this.value) && !util.isArray(this.value)) {
        // this is just the boxed out value 
    } else {
...
function getBoxOnlyOptions(options) {
    return util.copyFiltered.call(undefined, options, BOX_ONLY_PROPERTIES);
}
